...
export class APComponent implements OnInit {
  ...
  purchasesTotal:any;
  purchasesReturnTotal:any;
  monthlyPurchases:any;
  isEnglish:boolean=false;
  
  constructor(private _scriptservice : ScriptServiceService
    , private _activatedRoute: ActivatedRoute, private _userService: UserService
  ...
  LoadData()
  {
    this._DashboardService.GetAPTotalPurchases().subscribe((data) => {
      this.purchasesTotal=data.value;
    });
    this._DashboardService.GetAPTotalReturnPurchases().subscribe((data) => {
      this.purchasesReturnTotal=data.value;
    });
    this._DashboardService.GetAPMonthlyPurchases().subscribe((data) => {
      this.monthlyPurchases=data;
    });
  }
